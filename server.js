...
const TRADE_CHECK_INTERVAL_MS = 5000;  // Check every 5 seconds (respects rate limits)
const MIN_TRADE_SIZE_USD = 3;           // Minimum $3 per trade (Binance allows as low as ~$1-2)
const MAX_TRADE_SIZE_PERCENT = 15;      // Max 15% of balance per trade
...
// Calculate proper quantity based on asset's lot size
function roundQuantity(symbol, quantity) {
    ...
    if (symbol === 'SOLUSDT') return Math.floor(quantity * 100) / 100;
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    try {
        const { investmentAmount, targetAmount, profitPercent, timeLimitHours } = req.body;
        ...
        if (!profitPercent || profitPercent < 0.1 || profitPercent > 10) {
            return res.status(400).json({ success: false, message: 'Profit target per trade: 0.1% - 10%' });
        }
...
async function startRealTrading(sessionId) {
        ...
        try {
            if (trade.status === 'BUY_ORDER_PLACED') {
                ...
                if (orderStatus.status === 'FILLED') {
                    ...
                    const sellPrice = trade.fillPrice * (1 + session.profitPercent / 100);
                    ...
                    console.log(`📈 SELL ORDER PLACED: ${trade.filledQuantity} ${trade.symbol} at $${sellPrice}`);
    ...
    // Calculate how many trades to place
    ...
    // Calculate investment per trade (minimum $3)
    ...
    for (let i = 0; i < tradesToPlace; i++) {
        ...
        try {
            ...
            console.log(`📈 NEW BUY ORDER: $${investmentPerTrade.toFixed(2)} → ${roundedQty} ${symbol} at $${buyPrice.toFixed(2)} | Active: ${session.activeTrades.length}`);
